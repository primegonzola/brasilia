...
function display_help() {
	...
	echo -e "\t--frontend-image-uri, -fiu <frontend image uri>"
	echo -e "\t  the image to use for deploying frontend machines\n"
}
...
while true; do
  case "$1" in
    ...
    -i | --frontend-image-uri ) FRONTEND_IMAGE_URI="$2"; shift ; shift ;;
    -- ) shift; break ;;
    * ) break ;;
  esac
done
...
if [ -z ${FRONTEND_IMAGE_URI+x} ]
	then 
	display_help
	display_error "One or more missing or incorrect arguments\n"
	display_error "\terror: --frontend-image-uri, -i is missing.\n"
	echo -e "\tusage: --frontend-image-uri, -i [FRONTEND_IMAGE_URI]"
	echo -e "\n"
	exit 1; 
fi
